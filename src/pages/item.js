...
function ItemDetails() {
    ...
    const { data: item, itemError, itemLoading } = useFetch("http://localhost:8080/items/" + itemId)
    ...
    useEffect(() => {
        const fetchPriceHistory = async () => {
            try {
                const response = await fetch(`http://localhost:8080/items/${itemId}/price?size=${selectedSize}&color=${item.variationList[0].color}`);
